...
// parameters)
const correctJulia = dogs => dogs.slice(1, -1);

// 2. Create an array with both Julia's (corrected) and Kate's data
// 3. For each remaining dog, log to the console
// whether it's an adult ("Dog 🐶 number 1 is an adult, and is 5 years old")
...
function checkDogs(dogsJulia, dogsKate) {
  ...
  dogs.forEach((el, i) => {
    console.log(
      el >= 3
        ? `Dog 🐶 number ${i} is an adult, and is ${el} years old`
...
newDogsJulia = correctJulia(dogsJulia);
...
function calcAverageHumanAge(ages) {
  ...
  return (
    filteredHumanAges.reduce((acc, cur) => acc + cur) / filteredHumanAges.length
  );
}
// 4. Run the function for both test datasets
...
// Eating too much means
// the dog's current food portion is larger than the recommended portion,
// and eating too little is the opposite.
// Eating an okay amount means
// the dog's current food portion is
// within a range 10% above and 10% below the recommended portion (see hint).
const tooMuchFood = (curFood, recommendedFood) =>
  curFood > recommendedFood * 1.1 ? 'tooMuch' : null;
const tooLittleFood = (curFood, recommendedFood) =>
  curFood < recommendedFood * 0.9 ? 'tooLittle' : null;
const exactlyRecommended = (curFood, recommendedFood) =>
  curFood === recommendedFood ? 'exactlyRecommended' : null;

const checkFood = dog =>
  tooMuchFood(dog.curFood, dog.recommendedFood) ||
  tooLittleFood(dog.curFood, dog.recommendedFood) ||
  exactlyRecommended(dog.curFood, dog.recommendedFood) ||
  'recommended';
...
// Hint:
// Some dogs have multiple owners,
// so you first need to find Sarah in the owners array,
// and so this one is a bit tricky (on purpose) 🤓
console.log(checkFood(dogs.find(dog => dog.owners.includes('Sarah'))));

// 3. Create an array containing all owners of dogs
// who eat too much ('ownersEatTooMuch')
// and an array with all owners of dogs
// who eat too little ('ownersEatTooLittle').
const ownersEatTooMuch = dogs
  .filter(dog => checkFood(dog) === 'tooMuch')
  .map(dog => dog.owners);
const ownersEatTooLittle = dogs
  .filter(dog => checkFood(dog) === 'tooLittle')
  .map(dog => dog.owners);
...
let str =
  ownersEatTooMuch
    .map(owners => (owners.length > 1 ? owners.join(' and ') : owners))
...
// whether there is any dog eating exactly the amount of food
// that is recommended (just true or false)
console.log(!!dogs.find(dog => checkFood(dog) === 'exactlyRecommended'));

// 6. Log to the console
// whether there is any dog eating an okay amount of food
// (just true or false)
console.log(!!dogs.find(dog => checkFood(dog) === 'recommended'));

// 7. Create an array containing the dogs
// that are eating an okay amount of food
// (try to reuse the condition used in 6.)
const okFoodDogs = dogs.filter(dog => checkFood(dog) === 'recommended');
...
// Basically,the current portion should be
